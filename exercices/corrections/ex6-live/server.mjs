...
import { LOCAL_FILENAME, ROOT_PATH } from './utils/functions.mjs'
...
access(LOCAL_FILENAME, constants.F_OK, (err) => {
    if(err) {
        ...
        const child = fork(join(ROOT_PATH, 'child-process', 'countries.mjs'))
...
createServer(socket => {
    socket.on('error', (err) => {// configuration malsaine
        console.error('Error client from server', err)
        socket.destroy()
    })
    socket.on('close', () => { // sain
        console.log('Fin de la communication')
    })
})